...
| payment.reconcile | Payment reconciliation, evidence, review register, handoff-note read, handoff workload, owner SLA, active aging bucket, and stale evidence packet export endpoints | Finance Supervisor, Auditor Internal |
...
- Payment handoff workload, owner SLA, active aging bucket list/export, and stale evidence packet export visibility is gated by `payment.reconcile`; note create/revise controls remain separately gated by `payment.reconciliation.handoff-note`.
