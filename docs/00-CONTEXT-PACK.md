...
- Production authentication: Spring Security OAuth2 Resource Server 7.1.0 with Keycloak JWT; HTTP Basic is restricted to local/test profiles, while `oidc-smoke` exercises the resource server against an isolated non-production realm.
...
| BR-BIL-008 | Tariff and invoice snapshots separate water usage, fixed, levy, administration, waste, and prior-outstanding penalty; invoice subtotal excludes penalty while total/outstanding includes it | Billing | confirmed |
| BR-BIL-009 | Invoice issue debit must equal the sum of water, non-air, and penalty revenue credits; every positive component requires a revenue account and void reverses the exact posted lines | Billing/Accounting | confirmed |
...
| BR-UI-015 | User administration frontend must expose searchable users, identity status, role catalog, permission-disabled actions, mandatory audit reason, and complete loading/error/empty/pending feedback | Frontend/Admin | confirmed |
...
| REQ-BIL-001 | Kalkulasi tarif progresif valid | Billing | active effective tariff lookup combines progressive water blocks, versioned non-air charges, and penalty on prior outstanding; invoice persists every component snapshot | T-050 | TariffEngineApplicationServiceTest, InvoiceComponentSnapshotTest, TariffComponentMigrationTest |
...
| REQ-BIL-003 | Issue invoice menghasilkan jurnal piutang/pendapatan | Billing/Accounting | `/api/invoices/{id}/issue` calls AccountingApplicationService, debits total receivable and credits water/non-air/penalty revenue components with exact balance, then links invoice to the posted source journal | T-082 | BillingBatchApplicationServiceTest, AccountingApplicationServiceTest |
...
| REQ-SEC-005 | Administrasi user dan role terkendali | Security/Admin | `/api/admin/users` and `/api/admin/roles` use `user.read`; status and role replacement use separate permissions, mandatory reason, transactional row locking, IdP port, and before/after audit evidence | Gap Closure T4 | UserAdministrationServiceTest, UserAdminControllerPermissionTest, UserRoleAdminPermissionMigrationTest, admin-user-permissions.test.ts |
...
| 2026-07-12 | Split local Basic Auth from production Keycloak OIDC | Production credentials and bearer tokens must not be exposed through browser environment variables or local-only defaults | Spring Security Resource Server 7.1.0 is active in `prod` and isolated `oidc-smoke`; only the `prod` bean runs fail-fast HTTPS/secret validation. NextAuth 4.24.14 keeps the encrypted JWT session server-side and a same-origin BFF injects bearer tokens; `uuid` is pinned to 11.1.1 and npm audit reports zero vulnerabilities |
| 2026-07-12 | Add seeded release integration and operations gates | High-risk financial flows, period closing, OIDC wiring, and recovery procedures must be proven outside mocked unit tests before release | Gradle `integrationTest` uses fresh PostgreSQL/Testcontainers and deterministic API fixtures; Keycloak 26.7.0 smoke validates role/permission claims; CI runs both Compose smoke suites; production runbooks govern immutable deploy, PITR backup/restore, rollback, metrics, alerts, and financial control queries |
...
| 2026-07-08 | Route all browser API traffic through the same-origin BFF | Direct browser calls created CORS drift and exposed local Basic credentials through `NEXT_PUBLIC_*` variables | `/api/backend` forwards an allowlisted request surface; OIDC bearer and local-only Basic credentials are injected server-side, while browser bundles receive neither credential |
...
| 2026-07-10 | Add V24 blueprint performance indexes | Blueprint index review found the legacy payment/invoice indexes partly covered by V2, while current Spring services need broader list/report/reconciliation query support | Added additive PostgreSQL indexes for dashboard filters, open receivables, ledger date-range reports, metering, payment channel/status lists, and reconciliation review/workload paths |
...
| `scripts/smoke-compose.sh` with clean build | passed on 2026-07-12; backend health, anonymous auth, 20 frontend routes, Flyway history, V22/V23 constraints, and V24 indexes |
| `scripts/smoke-oidc.sh` with Keycloak 26.7.0 | passed on 2026-07-12; discovery, NextAuth provider, anonymous 401, token issuance, principal, realm/client roles, permission claim, and protected API |
| Full backend `gradle test` with Docker socket and PostgreSQL Testcontainers | passed on 2026-07-11 after V27 user/role administration; 3m53s |
| Admin runtime smoke (`/actuator/health`, `/api/auth/me`, `/api/admin/users`, `/api/admin/roles`, `/admin/users`) | passed on 2026-07-11; health UP, 58 admin authorities, 1 user, 14 roles, `LOCAL_ONLY`, frontend HTTP 200 |
| Admin browser smoke | passed on 2026-07-11; user and role data rendered, self-disable disabled, 14-role modal with audit reason, no console error or desktop overflow; mobile viewport override unavailable in browser runtime |
| `npm.cmd run test:permissions` | passed on 2026-07-11 after admin user permissions; 50 tests |
| `npm.cmd run typecheck`, `npm.cmd run lint`, `npm.cmd run build` | passed on 2026-07-11 after admin user workspace; 21 static routes |
| Full backend `gradle test` with Docker socket and PostgreSQL Testcontainers | passed on 2026-07-11 after V26 tariff component implementation; 3m54s |
| Focused fresh PostgreSQL migration verification (`TariffComponentMigrationTest`, `PreCloseRepositoryIntegrationTest`) | passed on 2026-07-11 after V26 historical invoice backfill; 3m20s |
| `npm.cmd run test:permissions` | passed on 2026-07-11 after billing component account validation; 48 tests |
| `npm.cmd run lint`, `npm.cmd run typecheck`, `npm.cmd run build` | passed on 2026-07-11 after tariff and billing frontend updates; 20 static routes |
| `docker compose up -d --build` plus browser smoke `/tariffs` and `/billing` | passed on 2026-07-11; health UP, authenticated admin has 55 authorities, desktop/mobile no horizontal overflow, console clean |
| `docker run --rm -v "D:\sia-pdam-enterprise\backend:/workspace" -w /workspace gradle:9.6.1-jdk26 gradle test --no-daemon` | passed on 2026-07-10 after V24 blueprint performance indexes |
| `npm.cmd run lint` | passed on 2026-07-10 after V24 blueprint performance indexes |
| `npm.cmd run typecheck` | passed on 2026-07-10 after V24 blueprint performance indexes |
| `npm.cmd run test:permissions` | passed on 2026-07-10 after V24 blueprint performance indexes; 47 tests |
| `npm.cmd run build` | passed on 2026-07-10 after V24 blueprint performance indexes; 20 static routes |
| Git Bash `bash.exe scripts/smoke-compose.sh` | passed on 2026-07-10 after V24 blueprint performance indexes; checked backend health, anonymous auth, 18 frontend routes, Flyway history, and V22/V23 constraints plus V24 index presence |
| Flyway schema history query | passed on 2026-07-10; V24 `blueprint performance indexes` success=true |
| `sh scripts/check-migration-constraints.sh` via Git for Windows `sh.exe` | passed on 2026-07-10; validates V22/V23 PostgreSQL constraints inside rollback-only transaction |
| `sh scripts/smoke-compose.sh` via Git for Windows `sh.exe` | passed on 2026-07-10 after migration constraint gate integration; checked backend health, `/api/auth/me`, 18 frontend routes, Flyway history, and V22/V23 constraints |
| `docker run --rm -v "D:\sia-pdam-enterprise\backend:/workspace" -w /workspace gradle:9.6.1-jdk26 gradle test --no-daemon` | passed on 2026-07-10 after V22/V23 constraint gate integration |
| `npm.cmd run lint` | passed on 2026-07-10 after V22/V23 constraint gate integration |
| `npm.cmd run typecheck` | passed on 2026-07-10 after V22/V23 constraint gate integration |
| `npm.cmd run test:permissions` | passed on 2026-07-10 after V22/V23 constraint gate integration; 47 tests |
| `npm.cmd run build` | passed on 2026-07-10 after V22/V23 constraint gate integration |
