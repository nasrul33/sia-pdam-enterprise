...
function providerLabel(status: AdminUser["identityProviderStatus"]): { label: string; tone: "neutral" | "success" | "danger" } {
  ...
  if (status === "EXTERNAL_MANAGED") return { label: "Dikelola IdP", tone: "neutral" };
...
export function AdminUserWorkspace() {
  ...
  const identityManagedUsers = users.filter((user) =>
    user.identityProviderStatus === "SYNCED" || user.identityProviderStatus === "EXTERNAL_MANAGED"
  ).length;
